  ...
  return <AppContext.Provider value={value}>
    <header className="sticky top-0 z-30 border-b border-emerald-950/10 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center gap-5 px-5 py-3">
        <Link href="/dashboard" className="flex items-center gap-2 text-xl font-black tracking-tight"><span className="grid h-9 w-9 place-items-center rounded-xl bg-emerald-700 text-white"><Landmark size={19}/></span>M-Link</Link>
        <nav className="hidden gap-1 md:flex"><Link className="rounded-lg px-3 py-2 text-sm font-semibold hover:bg-emerald-50" href="/dashboard">{t('dashboard')}</Link><Link className="rounded-lg px-3 py-2 text-sm font-semibold hover:bg-emerald-50" href="/customers">{t('customers')}</Link></nav>
