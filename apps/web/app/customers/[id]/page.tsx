      ...
      <div className="flex flex-col gap-5 p-5 md:flex-row md:items-center md:justify-between md:p-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-navy-500">CIF {c.customerCode} · {c.branch ?? '—'} · {t('customerSince')} {formatDay(c.customerSince.slice(0, 10), locale)}</p>
        ...
        <div className="flex items-center gap-5">
          {m && <div className="text-right"><div className="label">{t('priorityScore')}</div><div className="text-4xl font-black leading-tight text-orange-600">{m.priorityScore.toFixed(1)}</div>{asOf && <div className="flex items-center justify-end gap-1 text-[11px] text-navy-500">{t('computedAt')} {formatDay(asOf.asOfDate, locale)}{asOf.insufficientHistory && <AlertTriangle size={12} className="text-orange-600" aria-label={t('insufficientHistory')}/>}</div>}</div>}
          <button disabled={!ready || analyze.isPending} onClick={() => analyze.mutate()} className="button"><Sparkles size={18}/>{analyze.isPending ? t('analyzing') : t('analyze')}</button>
        </div>
      </div>
      <div className="grid grid-cols-2 divide-x divide-y divide-navy-100 border-t border-navy-100 md:grid-cols-4 md:divide-y-0">{kpis.map(([label, value]) => <div key={label} className="p-5"><div className="label">{label}</div><div className="mt-2 text-xl font-black">{money(value, locale)}</div></div>)}</div>
    </section>
...
function Overview({ customer, metrics, summary, accounts, interactions, positions, offers, period, locale, t }: { customer: Customer; metrics: CustomerMetrics | null; summary: PeriodSummary | undefined; accounts: Account[]; interactions: Interaction[]; positions: Position[]; offers: NextBestOffer[]; period: Period | null; locale: Locale; t: T }) {
  ...
  return <div className="grid gap-5 lg:grid-cols-3">
    ...
    <div className="card lg:col-span-2"><div className="card-head"><div><h2 className="text-base font-extrabold">{t('balanceTrend')}</h2>{period && <p className="text-xs text-navy-500">{formatDay(period.from, locale)} → {formatDay(period.to, locale)}</p>}</div></div><div className="h-64 p-5"><ResponsiveContainer width="100%" height="100%"><AreaChart data={trend}><defs><linearGradient id="balance" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#091e42" stopOpacity={.28}/><stop offset="95%" stopColor="#091e42" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="date" fontSize={11} interval={interval}/><YAxis fontSize={11} unit="M"/><Tooltip/><Legend/><Area type="monotone" dataKey="casa" name="CASA" stroke="#091e42" fill="url(#balance)" strokeWidth={2}/><Area type="monotone" dataKey="fd" name="FD" stroke="#f4600c" fill="none" strokeWidth={1.5}/><Area type="monotone" dataKey="invest" name="Bond + CCQ" stroke="#7d8ca8" fill="none" strokeWidth={1.5}/></AreaChart></ResponsiveContainer></div></div>
    <div className="card"><div className="card-head"><h2 className="text-base font-extrabold">{t('spendingCategories')}</h2></div><div className="h-64 p-5">{spending.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={spending} layout="vertical"><XAxis type="number" hide/><YAxis type="category" dataKey="name" width={120} fontSize={10}/><Tooltip/><Bar dataKey="value" name="M VND" fill="#f4600c" radius={[0,6,6,0]}/></BarChart></ResponsiveContainer> : <Empty message={t('empty')}/>}</div></div>
  </div>;
}
